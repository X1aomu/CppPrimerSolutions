#include "12/32_text_query_with_strblob.h"
...
void TextQuery::GenerateResult() {
  LineNo n = 0;
  for (auto line = (*file_input_).begin(); line != (*file_input_).end();
       line.incr()) {
    istringstream iss(line.deref());
    string word;
    while (iss >> word) {
...
std::ostream &QueryResult::Print(std::ostream &os) {
  ...
  for (auto line_no : *line_nos_ptr_) {
    ConstStrBlobPtr line_ptr(*file_ptr_, line_no);
    os << "\t"
       << "(line " << line_no + 1 << ") " << line_ptr.deref() << std::endl;
  }
  return os;
}
